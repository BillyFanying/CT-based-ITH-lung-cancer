...
with open("E:\\DATA\\TCGA-CT\\SCGA.json", 'r') as load_f:
...
for pathdicom in pathlist:
  ...
  for listnum in listpic:
    if 'CT_nii' in listnum and 'nrrd' in listnum:
      path_nii_nrrd = os.path.join(pathdicom, listnum)
      maskName = path_nii_nrrd
      break
  for listnum in listpic:
    if 'CT_dcm' in listnum and 'nrrd' in listnum:
  ...
  path_fea_all = r'E:/DATA/TCGA-CT/feature/Chemi_TCGA.xlsx'

  print('Folder currently being processed: ', PathDicom)
  ...
  try:
      ...
      print("Start writing to spreadsheet")
      ...
      Ecnt+=1
      print('Processing error file number: ', Ecnt)
